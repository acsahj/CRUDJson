...
from src.load_todo import TodoManager

def test_load_list_success():
    #Tests that load_list reads and returns valid JSON data from the file path which is predefined.
    mock_data='{"todo": [{"id": 1,"task": "Test the method"}]}'
    with patch("builtins.open",mock_open(read_data=mock_data)):
        with patch("json.load",return_value={"todo": [{"id": 1,"task": "Test the method"}]}):
            result= TodoManager.load_list()
